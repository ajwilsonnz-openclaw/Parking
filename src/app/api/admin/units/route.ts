...
export async function PUT(req: NextRequest) {
  ...
  if (!id || !unit_number) return NextResponse.json({ error: 'ID and unit number required' }, { status: 400 });

  const cleanUnit = String(unit_number).trim();
  const parksNum = parseInt(assigned_parks) || 1;

  await execDb(
    'UPDATE units SET unit_number = ?, assigned_parks = ?, notes = ? WHERE id = ?',
    [cleanUnit, parksNum, notes || null, id]
  );
